...
# 5G MEC UPLINK Performance Evaluation Script.
# Executed inside the UPF VM (192.168.6.50) against MEC HOST VM (192.168.6.10)

# "baseline" or "ebpf_xdp_enabled"
TEST_MODE="baseline"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")

TARGET_IP="192.168.6.10"
...
RAW_DIR="raw_results_uplink_${TEST_MODE}_${TIMESTAMP}"
mkdir -p "$RAW_DIR"
...
echo "========================================================================="
echo "Starting 5G MEC Network Evaluation against $TARGET_IP in UPLINK direction"
...
for SIZE in "${PACKET_SIZES[@]}"; do
    ...
    echo "-> Phase 1: Running iperf3 (UDP Throughput & PPS) in UPLINK direction..."
    for ((i = 1; i <= ITERATIONS; i++)); do
        ...
        echo "$RAW_JSON" >"$RAW_DIR/iperf_uplink_${SIZE}B_run${i}.json"
    ...
    echo "-> Phase 2: Running sockperf (UDP Latency under load) in UPLINK direction..."
    for ((i = 1; i <= ITERATIONS; i++)); do
        echo -n "   Run $i/$ITERATIONS: "

        RAW_OUTPUT=$(
            sockperf under-load -i "$TARGET_IP" -p "$SOCKPERF_SERVER_PORT" --client_port "$SOCKPERF_CLIENT_PORT" \
                -m "$SIZE" -t "$DURATION" --full-log "$RAW_DIR/sockperf_uplink_${SIZE}B_run${i}_full.log" 2>&1
        )
        ...
        echo "$RAW_OUTPUT" >"$RAW_DIR/sockperf_uplink_${SIZE}B_run${i}_summary.txt"
    ...
    echo "------------------------------------------------------------"
    echo "UPLINK RESULTS FOR $SIZE BYTES (Median of $ITERATIONS runs)"
    echo "------------------------------------------------------------"
